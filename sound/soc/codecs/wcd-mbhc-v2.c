		      ...
		      bool impedance_det_en)
{
...
#ifdef CONFIG_TAS2557_EXIST_D3
	max_enable_gpio = of_get_named_gpio(card->dev->of_node, "qcom,max97220-enable-gpio",0);
	printk("%s: max_enable_gpio = %d\n",__func__, max_enable_gpio);
	if (max_enable_gpio <0) {
		dev_err(card->dev,
	...
	if (gpio_is_valid(max_enable_gpio)) {
		...
		gpio_direction_output(max_enable_gpio, 0);
		msleep(5);
	}
#endif
#if defined (CONFIG_UGGLITE) || defined (CONFIG_UGG)
